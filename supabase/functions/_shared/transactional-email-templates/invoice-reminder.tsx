...
const InvoiceReminder = ({
  ...
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>{`Reminder: invoice ${documentNumber} from ${companyName} is awaiting payment`}</Preview>
    <Body style={main}>
      <Container style={container}>
        ...
        <Heading style={heading}>Friendly payment reminder</Heading>

        <Section style={bodySection}>
          <Text style={bodyText}>Hi {clientName},</Text>
          <Text style={bodyText}>
            Just a quick reminder that invoice <strong>{documentNumber}</strong>
            {amount ? <> for <strong>{amount}</strong></> : null} is still awaiting payment
            {dueDate ? <> (due {dueDate})</> : null}.
          </Text>
          <Text style={bodyText}>
            You can review and pay it securely online using the button below.
          </Text>
        </Section>
        ...
        <Text style={footer}>
          Sent via {SITE_NAME} on behalf of {companyName}. If you’ve already paid, please disregard this message.
        </Text>
      </Container>
    </Body>
  </Html>
...
export const template = {
  ...
  subject: (data: Record<string, any>) =>
    `Reminder: invoice ${data.documentNumber || ''} from ${data.companyName || 'Us'}`.trim(),
