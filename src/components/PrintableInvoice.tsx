    ...
    <div className="print-document p-8 max-w-[800px] mx-auto bg-background text-foreground font-sans text-sm">
      ...
      <div className="flex justify-between items-start mb-8 border-b border-border pb-6">
        ...
        <div className="text-right">
          ...
          {invoice.title && <p className="text-muted-foreground text-xs mt-1">{invoice.title}</p>}
        </div>
      </div>
      ...
      <div className="flex justify-between mb-8">
        ...
        <div className="text-right text-xs space-y-0.5">
          <div><span className="text-muted-foreground">Date: </span>{format(new Date(invoice.created_at), "MMM d, yyyy")}</div>
          {invoice.due_date && <div><span className="text-muted-foreground">Due: </span>{format(new Date(invoice.due_date), "MMM d, yyyy")}</div>}
          {invoice.payment_terms && <div><span className="text-muted-foreground">Terms: </span>{invoice.payment_terms}</div>}
          <div className="mt-2">
            <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium uppercase ${
              invoice.status === "paid" ? "bg-emerald-100 text-emerald-800" : 
              invoice.status === "overdue" ? "bg-red-100 text-red-800" : "bg-muted text-muted-foreground"
            }`}>
              {invoice.status}
            </span>
          </div>
      ...
      <div className="flex justify-end mb-8">
        <div className="w-[250px] space-y-1 text-sm">
          ...
          {Number(invoice.amount_paid) > 0 && (
            <>
              <div className="flex justify-between"><span className="text-muted-foreground">Paid</span><span>${Number(invoice.amount_paid).toFixed(2)}</span></div>
              <div className="flex justify-between font-bold"><span>Balance Due</span><span>${Number(invoice.balance_due).toFixed(2)}</span></div>
            </>
          )}
        </div>
      </div>
      ...
      {invoice.client_notes && (
        <div className="border-t border-border pt-4">
          ...
          <p className="text-xs text-muted-foreground whitespace-pre-wrap">{invoice.client_notes}</p>
        </div>
      )}
    </div>
  );
